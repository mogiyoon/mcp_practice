...
실험 컴포넌트들을 조합해 결과를 관찰합니다.

동작 흐름:
  1. Claude가 exp_select / exp_add 로 컴포넌트를 선택한다.
  2. exp_run() 을 호출하면 선택된 컴포넌트의 태그/이름이
     REACTION_RULES 와 매칭되어 결과 텍스트가 생성된다.
  3. 결과는 state/experiment.txt 에 저장되고 display.py 가 실시간으로 표시한다.
"""
...
class ExpComponent:
    ...
    tags:       반응 규칙 매칭에 쓰이는 분류 키워드 목록.
    properties: 수치 속성 — run_experiment() 에서 합산해 안정성 점수를 계산한다.
    """
    name:        str
    category:    str          # "물질" | "환경" | "촉매" | "에너지"
    description: str
    tags:        list[str]
    properties:  dict         # 예: {"반응성": 8, "온도영향": 3}


# 전체 컴포넌트 딕셔너리 — 이름으로 빠르게 조회
...
# 반응 규칙 테이블: (매칭 조건 세트, 반응 이름, 결과 설명)
# 매칭 조건은 컴포넌트 이름 집합 또는 태그 집합에 대해 issubset으로 판단한다.
# 즉, 조건 세트가 선택된 이름/태그 중에 모두 포함되어 있으면 반응이 발동한다.
...
# ───────────────────────────────────────────────
# 실험 세션 상태 (프로세스가 살아있는 동안 유지)
# ───────────────────────────────────────────────

_session: dict = {
    "selected": [],   # 현재 선택된 컴포넌트 이름 목록
    "results":  [],   # 실험 결과 기록 (history용)
}
...
def select_components(names: list[str]) -> str:
    """실험에 사용할 컴포넌트를 선택합니다 (기존 선택을 초기화한다)."""
    valid, invalid = [], []
    for name in names:
...
def run_experiment() -> str:
    """현재 선택된 컴포넌트들로 실험을 실행하고 결과를 반환합니다.

    실행 단계:
      1. 선택된 컴포넌트의 모든 태그와 이름을 수집한다.
      2. 속성 값을 합산한다 (반응성, 안정성 등).
      3. REACTION_RULES를 순회하며 매칭되는 반응을 찾는다.
      4. 종합 안정성 점수를 계산한다: 안정성 - 반응성 // 2
    """
    ...
    # REACTION_RULES에서 매칭되는 반응을 찾는다.
    # 규칙의 조건 세트가 이름 집합 또는 태그 집합의 부분집합이면 반응 발동
    matched_reactions = []
